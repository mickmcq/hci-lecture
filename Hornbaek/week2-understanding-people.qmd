...
- **It powers real systems:** key-target resizing in phone keyboards and layout-optimization algorithms that challenge QWERTY (**MacKenzie**, **Zhai**) are Fitts' Law applied.
...
## Some target-acquisition fails

::: {.callout-warning title="One Fitts' Law, or several?"}
- The clean **MT = a + b·ID** is presented as settled, and it is the most-tested model in HCI.
- But throughput has *no* agreed definition: **MacKenzie** ties it to average ID over MT; **Zhai** ties it to the slope 1/b alone. Each throws away different information.
- The model is also brittle: fit it on 5 to 10 cm targets and it may not generalize to 10 to 20 cm; 2D and touch need extensions (**MacKenzie**), and corrective-submovement models (**Meyer et al.**) explain what the average hides.
- Design takeaway: use Fitts' Law to reason and to compare within a study; do not quote one lab's a and b as universal constants.
:::
...
- Several distinct memory systems run at once (**Squire**): working memory, and long-term memory split into declarative (facts, events) and non-declarative (skills, priming, conditioning).
...
- **Knowledge-in-the-world beats knowledge-in-the-head (Norman):** show the options instead of demanding memorized commands, and you offload cognition onto the display.
...
- **Failure is a model clash:** most "user error" is the designer's model and the user's model disagreeing, exactly the kiosk problem.
- **AI raises the stakes:** with an AI agent acting on your behalf, the user's model of "what will it do" is thin and often wrong. A **Bayesian-brain** view says users constantly update predictions ("what will autocomplete do next?"), so systems must make their behavior *predictable and legible*.
...
- **Prospect theory (Tversky & Kahneman):** choices are judged against a reference point, and losses hurt more than equivalent gains: loss aversion. Framing a change as a loss vs a gain flips behavior.
...
**Task:** In groups of 3 to 4, pick one real screen (a checkout, a booking flow, a settings page) and audit it against Video 1 and Video 2.

**Produce:** An annotated screenshot table with at least 6 rows: for each, name the element, its intended importance, its actual saliency (what draws the eye first), and its Fitts cost (rough D and W for the primary action). Include at least one row flagged "this design decision is wrong, and here is our argument."
...
**Produce:** A two-column table with at least 6 rows: "what users predict it will do" vs "what it actually does," plus a legibility fix for each mismatch. End with one position statement: name one place the product *exploits* a heuristic (availability, status quo, loss framing) and argue whether that is acceptable.
