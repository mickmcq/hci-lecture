...
- **Measurement is real:** the PENS scale gauges competence, autonomy, and relatedness in games, so "needs" is not just hand-waving
- The rule: name the need a feature serves *before* you build it; a feature that serves no need buys you no intrinsic motivation
...
- **Callback:** an extrinsic counter bolted onto an intrinsically social act
...
- **Callback:** the streak is a nudge too, just one pointed at the designer's retention target rather than your goal
...
- **Transparency is the pivot (Caraban):** a nudge the user can see and understand is far less ethically fraught than a hidden one
- **Callback:** Snapstreaks sit right on this line, playful engagement to some, engineered compulsion aimed at minors to others
- Design takeaway: if a mechanic only works when the user does not notice it, that is your ethics alarm

::: {.callout-warning title="Not so fast..."}
- **Fogg** and early persuasive-design work framed influence as basically benign, just applying persuasion to software
...
- **Callback:** the streak's daily grind is forgettable; the panic of almost losing it is a vivid peak, which is exactly why it sticks
...
- **Hedonic quality (Hassenzahl):** what the system does for the self beyond the task
...
- **Outeraction (Nardi):** the "you there?" pings that set up conditions for real exchange are not overhead, they are the glue
...
- **Callback:** the streak worked because both people paid in; a tool where only one side benefits dies
...
- **Callback:** the "Fine." problem is why "Fine 🙂" and "Fine." now feel like different messages
...
- **CASA (Nass):** people apply human social rules to computers automatically, even knowing better
...
- **The critique of anthropomorphism:** copying the human form can limit innovation (the first cars had reins before someone invented the wheel), can dampen users' sense of control and responsibility, and can blur what machines can actually do
